...
#set bounds of transformation parameters
...
#change image gamma for registration
...
def gamma_correction2(img1, img2):
    rand1 = random.uniform(.05,4)
    rand2 = random.uniform(.05, 4)
    transformed_bright1 = exposure.adjust_gamma(img1, rand1)
...
def transform_gen(select, init, batch_size, l,f):
    ...
    x_tester=np.zeros([batch_size,192,144,1])
    ...
    for i in range(0, batch_size):
        a, b, c = roll(120, 70, 25, 20, 2, -2)
        kappa=f[i]
        zed=l[i]
        warp,m,n,M= elastic_transform(select[i,:,:], a, b, c)
        ref_warp=elastic_transform(init[i,:,:],a,b,c)
        mov, ref = gamma_correction2(warp, init[i,:,:])
        x_train_norm[alpha, :, :, 0] = warp
        x_train_norm[alpha, :, :, 1] = init[i,:,:]
        x_tester[alpha,:,:,0]=ref+cv2.randn(ref,0,.0005)
        alpha = alpha + 1
